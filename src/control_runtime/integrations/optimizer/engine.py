...
class SliceOptimizationEngine:
    ...
    def _slice_kpis_for_constraints(self, slice_obj: Slice) -> Tuple[float, float, float, float]:
        source = str(getattr(self.config, "slice_kpi_source", "qos") or "qos").strip().lower()
        if source == "qos":
            return (
                float(slice_obj.qos.latency or 0.0),
                float(slice_obj.qos.processing_delay or 0.0),
                float(slice_obj.qos.jitter or 0.0),
                float(slice_obj.qos.loss_rate or 0.0),
            )
        if source == "telemetry":
            latency = slice_obj.telemetry.latency
            jitter = slice_obj.telemetry.jitter
            loss_rate = slice_obj.telemetry.loss_rate
            if latency is None or jitter is None or loss_rate is None:
                raise ValueError(
                    f"slice telemetry KPI is incomplete for slice {slice_obj.snssai}; "
                    "cannot enforce telemetry-based SLA constraints"
                )
            return (
                float(latency),
                0.0,
        ...
        raise ValueError(f"unsupported slice_kpi_source: {source}")

    def _slice_modeled_baseline_loads(self, apps: List[App]) -> Dict[str, Dict[str, float]]:
        """统计当前优化问题中已显式建模的旧分配负载。"""
        ...
        for app in apps:
            for flow in app.flows:
                ...
                if not slice_snssai:
                    continue
                item = baseline.setdefault(slice_snssai, {"ul": 0.0, "dl": 0.0})
                item["ul"] += float(flow.allocation.allocated_bandwidth_ul or 0.0)
                item["dl"] += float(flow.allocation.allocated_bandwidth_dl or 0.0)
        return baseline

    def _effective_background_load(
        self,
        slice_obj: Slice,
        modeled_baseline_loads: Dict[str, Dict[str, float]],
    ) -> Tuple[float, float]:
        """仅保留未被 flow allocation 显式建模的背景负载。"""
        baseline = modeled_baseline_loads.get(slice_obj.snssai, {})
        modeled_ul = float(baseline.get("ul", 0.0) or 0.0)
        modeled_dl = float(baseline.get("dl", 0.0) or 0.0)
        raw_bg_ul = float(slice_obj.load.current_bandwidth_ul or 0.0)
        raw_bg_dl = float(slice_obj.load.current_bandwidth_dl or 0.0)
        return max(0.0, raw_bg_ul - modeled_ul), max(0.0, raw_bg_dl - modeled_dl)
    ...
    def _finalize_results(self, prob, apps, slices, x, B_act_ul, B_act_dl):
        ...
        if status_str != "Optimal":
            logger.warning(
                f"Optimizer produced non-optimal status={status_str}; suppressing executable result serialization"
            )
            return pd.DataFrame(), pd.DataFrame(), status_str, None
            ...
            for f in app.flows:
                ...
                for s in slices:
                    ...
                    if self.config.enable_sla_constraints:
                        ...
                        violates_jitter = f.sla.jitter > 0 and slice_jitter > f.sla.jitter
                        violates_loss = f.sla.loss_rate > 0 and slice_loss > f.sla.loss_rate
    ...
    def _add_slice_constraints(self, prob, apps, slices, B_act_ul, B_act_dl, dev):
        modeled_baseline_loads = self._slice_modeled_baseline_loads(apps)
        for s in slices:
            background_ul, background_dl = self._effective_background_load(s, modeled_baseline_loads)
            # C6.1: 上行切片容量约束
            real_avail_ul = (
            ...
            load_ratio_ul = (
                (total_allocated_ul + background_ul)
                / s.capacity.total_bandwidth_ul
                if s.capacity.total_bandwidth_ul > 0 else 0
            )
            load_ratio_dl = (
                (total_allocated_dl + background_dl)
                / s.capacity.total_bandwidth_dl
    ...
    def _format_slice_stats(self, apps, slices, B_act_ul, B_act_dl) -> pd.DataFrame:
        ...
        for s in slices:
            ...
            # 总负载 = 动态分配 + 未被显式建模的背景负载
            total_load_ul = allocated_to_flows_ul + background_ul
            total_load_dl = allocated_to_flows_dl + background_dl
