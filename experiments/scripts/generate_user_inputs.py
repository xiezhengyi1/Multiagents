...
from control_runtime.integrations.storage import get_latest_snapshot_data
...
def _ensure_supi_suffix(user_input: str, supi: str) -> str:
    text = str(user_input or "").strip()
    normalized_supi = str(supi or "").strip()
    if not text or not normalized_supi:
        return text
    suffix = f"(supi: {normalized_supi})"
    lowered = text.lower()
...
def build_templates(anchor: Dict[str, Any]) -> List[UserInputCandidate]:
    ...
    return [
        UserInputCandidate(
            user_input=_ensure_supi_suffix(user_input, supi),
            messages=_single_turn_messages(_ensure_supi_suffix(user_input, supi)),
        )
