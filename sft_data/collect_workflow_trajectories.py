...
from workflows.main_control_orchestrator import MainControlOrchestrator
...
def load_user_input_records(
    *,
    ...
    if user_inputs_path is not None:
        ...
        return [_normalize_record(record, index) for index, record in enumerate(raw_records, start=1)]
...
def _store_projected_trajectories(
    *,
    ...
    traces_by_agent = load_projected_traces_by_agent(PROJECT_ROOT, agent_names=WORKFLOW_AGENT_NAMES)
